...
class SessionStats:
    ...
    def get_specific_summary(self,number):
        ...
        if(number>len(text)-2):
            print("There are only "+str(len(text)-2)+" chat sessions, choose a valid number next time")
            return
        line=text[number]
        data=line.split(",")
